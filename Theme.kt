package com.example.ui.theme

import android.os.Build
import androidx.compose.foundation.isSystemInDarkTheme
import androidx.compose.material3.MaterialTheme
import androidx.compose.material3.darkColorScheme
import androidx.compose.material3.dynamicDarkColorScheme
import androidx.compose.material3.dynamicLightColorScheme
import androidx.compose.material3.lightColorScheme
import androidx.compose.runtime.Composable
import androidx.compose.ui.graphics.Color
import androidx.compose.ui.platform.LocalContext

private val DarkColorScheme =
  darkColorScheme(
    primary = FieldGreenDarkTheme,
    onPrimary = FieldGreenDark,
    primaryContainer = FieldGreenDark,
    onPrimaryContainer = Color(0xFFC7F3D6),
    secondary = WheatGoldDarkTheme,
    onSecondary = SoilBrown,
    secondaryContainer = Color(0xFF563D00),
    onSecondaryContainer = Color(0xFFFFDEA3),
    tertiary = SkyWeatherBlue,
    background = AgriSurfaceDark,
    surface = AgriCardBgDark,
    surfaceVariant = Color(0xFF26332A),
    onBackground = AgriTextLight,
    onSurface = AgriTextLight
  )

private val LightColorScheme =
  lightColorScheme(
    primary = FieldGreenPrimary,
    onPrimary = Color.White,
    primaryContainer = Color(0xFFC4EED0),
    onPrimaryContainer = FieldGreenDark,
    secondary = WheatGold,
    onSecondary = Color.White,
    secondaryContainer = Color(0xFFFFF0CB),
    onSecondaryContainer = Color(0xFF5C3F00),
    tertiary = LeafTeal,
    background = AgriSurfaceLight,
    surface = AgriCardBgLight,
    surfaceVariant = AgriSurfaceVariantLight,
    onBackground = AgriTextDark,
    onSurface = AgriTextDark
  )

@Composable
fun MyApplicationTheme(
  darkTheme: Boolean = isSystemInDarkTheme(),
  // Dynamic color is available on Android 12+
  dynamicColor: Boolean = true,
  content: @Composable () -> Unit,
) {
  val colorScheme =
    when {
      dynamicColor && Build.VERSION.SDK_INT >= Build.VERSION_CODES.S -> {
        val context = LocalContext.current
        if (darkTheme) dynamicDarkColorScheme(context) else dynamicLightColorScheme(context)
      }

      darkTheme -> DarkColorScheme
      else -> LightColorScheme
    }

  MaterialTheme(colorScheme = colorScheme, typography = Typography, content = content)
}
